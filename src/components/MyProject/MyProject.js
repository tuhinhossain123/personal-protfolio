...
const MyProject = () => {
  ...
  const ui = data.filter(
      (singleData) => singleData.title === "UI/UX"
    );
  const web = data.filter(
      (singleData) => singleData.title === "Web Design"
    );
  const app = data.filter(
      (singleData) => singleData.title === "App Design"
    );
        ...
        <Tabs defaultIndex={2} onSelect={(index) => setTabIndex(index)}>
          <div className="">
            <TabList className="flex items-center justify-center space-x-4 cursor-pointer text-[24px] font-poppins font-normal text-[#000] pt-5 lg:pt-0">
              ...
              <Tab className={"menuItem bg-[#F8F8F8] focus:bg-primary focus:text-white text-2xl py-[10px] px-5"}>App Design</Tab>
              <Tab className={"menuItem bg-[#F8F8F8] focus:bg-primary focus:text-white text-2xl py-[10px] px-5" }>Graphic Design</Tab>
            </TabList>
          </div>
